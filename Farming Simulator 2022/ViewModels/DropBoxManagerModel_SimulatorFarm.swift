           ...
           .response(completionHandler: { [weak self] response, error in
               ...
               if let response = response {
                   do {
                       ...
                       if fileContents.count != self.skinsDataCount || self.coreDataHelper.getSkins_SimulatorFarm().isEmpty {
                           ...
                           print("Using existing skins data")
                       }
                   ...
                   print("Error downloading skins: \(error)")
               }
           })
           .progress({ progress in
           ...
           .response(completionHandler: { [weak self] response, error in
               ...
               if let response = response {
                   do {
                       ...
                       // Check if data needs updating or if Core Data is empty
                       if fileContents.count != self.farmsDataCount || self.coreDataHelper.getFarms_SimulatorFarm().isEmpty {
                           ...
                           print("Using existing farms data")
                       }
                       
                       self.progress += 25

                   } catch {
                       print("Error processing farms: \(error)")
                   }
               } else if let error = error {
                   print("Error downloading farms: \(error)")
               }
           })
           .progress({ progress in
               print("Download progress: \(Int(progress.fractionCompleted * 100))%")
